...
Future<bool> tableExists(int tableId) async {
  var manager_api = "http://localhost:8081/";
  var header = {};
  const website = "http://localhost:8000/";
  try {
    var response = await http
        //, headers: headers
        .get(manager_api + 'tables/$tableId')
...
Future<List<Item>> fetchItems() async {
  try {
    ...
    List<Item> items = [];
    var response = await http
        .get(manager_api + 'items/', headers: headers)
        .timeout(Duration(seconds: 3));

    if (response.statusCode != HttpStatus.ok) return items;
...
Future<List<Type>> fetchTypes() async {
  try {
    ...
    List<Type> types = [];
    var response = await http
        .get(manager_api + 'types/', headers: headers)
        .timeout(Duration(seconds: 3));

    if (response.statusCode != HttpStatus.ok) return types;
...
Future<bool> postOrder(int tableId, List<Item> items) async {
  try {
    ...
    var response = await http
        .post(manager_api + 'items/',
            headers: jsonHeaders, body: jsonEncode(orderedItemsList.toJson()))
